...
describe UserNotifierMailer, type: :mailer do
  it 'sends a mailer' do
    ...
    user = create(:user, username: 'TestUsername', confirm_token: "TestToken123")
    ...
    expect(email.parts[0].body.to_s).to eq(text_email_fixture)

    expect(email.parts[1].content_type.to_s).to eq('text/html; charset=UTF-8')
    expect(email.parts[1].body.to_s).to eq(html_email_fixture)
  end
end
